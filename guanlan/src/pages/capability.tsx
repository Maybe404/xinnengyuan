          ...
          <Card key={g} className="rise" style={{ animationDelay: `${gi * 70}ms` }}>
            ...
            <CardContent className="space-y-1.5">
              {CAP_FIELDS.filter((f) => f.group === g).map((f) => (
                <div key={f.name} className="rounded-lg bg-white/50 px-3 py-2">
      ...
      <Card className="rise">
        ...
        <CardContent className="space-y-3">
          <div className="flex flex-wrap items-center gap-2.5 text-[12.5px]">
            <span className="text-muted-foreground">示例字段：</span>
            <span className="font-medium">本地运维团队（江苏）</span>
            <span className="text-muted-foreground/60">旧值「外包过渡方案」→ 新值「无 → 成立条件」</span>
            <Button
              size="sm"
              ...
              onClick={() => {
                ...
                toast(
                  edited
                    ? "已还原演示编辑"
                    : "能力字段新版本已保存（CAP-2026-09.1）· 已生成变更事件并反查依赖"
                )
              }}
            >
              {edited ? "还原编辑" : "模拟编辑此字段"}
          ...
          {(edited || true) && (
            <div className="rounded-xl bg-white/55 p-3.5">
